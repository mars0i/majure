...
(defrecord InstanceState [yard
                          buddies
                          longz      ; [num-students]
                          doublez    ; [force-to-school-multiplier, random-multiplier] 
                          booleanz]) ; [tempering]
;; Indexes into arrays in the InstanceState:
(def ^:const +longz-num-students-idx+ 0)
(def ^:const +doublez-force-to-school-multiplier-idx+ 0)
(def ^:const +doublez-random-multiplier-idx+ 1)
(def ^:const +booleanz-tempering-idx+ 0)

(defn -init-instance-state
  [seed]
  [[seed] (InstanceState. (Continuous2D. 1.0 100 100)
                          (Network. false)
                          (long-array [50])         ; [num-students]
                          (double-array [0.01 0.1]) ; [force-to-school-multiplier, random-multiplier]
                          (boolean-array [true]))]) ; [tempering]
...
(defn -gitYard ^Continuous2D
  [^Students this]
  (:yard ^InstanceState (.instanceState this)))

(defn -gitBuddies ^Network
  [^Students this]
  (:buddies ^InstanceState (.instanceState this)))

(defn -getNumStudents
  ^long [^Students this]
  (aget ^longs (:longz ^InstanceState (.instanceState this))
        +longz-num-students-idx+))

(defn -setNumStudents
  [^Students this ^long newval]
  (when (> newval 0)
    (aset-long (:longz ^InstanceState (.instanceState this))
               +longz-num-students-idx+
               newval)))

(defn -getForceToSchoolMultiplier
  ^double [^Students this]
  (aget ^doubles (:doublez ^InstanceState (.instanceState this))
        +doublez-force-to-school-multiplier-idx+))

(defn -setForceToSchoolMultiplier
  [^Students this ^double newval]
  (when (>= newval 0.0)
    (aset-double (:doublez ^InstanceState (.instanceState this))
                 +doublez-force-to-school-multiplier-idx+
                 newval)))
...
(defn -isTempering
  ...
  (aget ^booleans (:booleanz ^InstanceState (.instanceState this))
        +booleanz-tempering-idx+))

(defn -setTempering
  [^Students this ^java.lang.Boolean newval]
  (aset-boolean (:booleanz ^InstanceState (.instanceState this))
                +booleanz-tempering-idx+
                newval))
